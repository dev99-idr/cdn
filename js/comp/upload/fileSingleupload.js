...
var gv_FileListDnDInfo = "<tr><td class=\"dndRow\" colspan=\"6\" style=\"height:50px\"  align=\"center\">※ Drag to attach <br>※ Only one file can be registered.<br>※ Delete : delete -> Save File -> X button</td></tr>";
...
var gv_UploadButton = "<button type='button' class='btn btn-primary btn-sm' id='btnSave' onclick=\'gf_onFileUpload()\'>Save File</button>";
...
var gv_OpenButton = "<button type='button' class='btn btn-light btn-sm' id='openFileBtn' onclick=\'gf_onFileSelect()\'>Open File</button>";
...
//var gv_InitButton = "<button type='button' class='btn btn-light btn-sm' onclick=\'gf_onFileInit()\' style=\'margin:3px 5px;\'>작업취소</button>";
// div string
...
function dragEnter(evt) {
	if(gds_FileList.size() > 0) {
		gf_AlertMsg('You cannot register more than one file.');
...
function drop(evt) {
    ...
    if (gv_Files.length == 0) {
        ...
        if (gf_checkDupFile(files)) {
            gf_AlertMsg('The same file is registered.');
            return;
        } else {
            // 중복 안됬으면 배열에 푸시
            gf_pushArray(files);
    ...
    if (typeof(FileList) == "undefined") { //file input single만 지원한경우
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (totalSize > 83886080) {
                gf_AlertMsg("The total file size that can be attached is 80 megabytes.\r\nFiles larger than 80 megabytes are excluded from attachments.");
                break;
            }
            var fileTableRow = "<tr id='" + (trObj.size() + 1) + "' > 					\r\n " +
                "	<td>" + (trObj.size() + 1) + "</td> 									\r\n " +
...
function gf_deleteFile(obj, fileId) {
    ...
    if (!gf_IsNull(fileId)) {
        if (!confirm('Files are also deleted. Would you like to continue?\n※When deleted, the file cannot be recovered.')) {
            return;
        }
    } else {
...
function gf_onFileUpload(type) {
    ...
    if (typeof(FileList) == "undefined") { //file single만 지원시
        if ($("input[name='file']").size < 2) {
            gf_Trace("There are no files to upload.");
        ...
        if(gv_Files.length > 1) {
        	gf_AlertMsg("You cannot upload more than one file.");
        	return false ;
        }
        ...
        v_FileNm = gds_FileList._rows[0]._data.fileNm;
